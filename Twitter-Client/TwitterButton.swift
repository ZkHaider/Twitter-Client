...
public class TwitterButton: UIButton {
    ...
    private func createLayers(image: UIImage!) {
        ...
        let imageFrame = CGRect(x: frame.size.width / 2 - frame.size.width / 4, y: frame.size.height / 2 - frame.size.height / 4,
                                width: frame.size.width / 2, height: frame.size.height / 2)
        let imgCenterPoint = CGPoint(x: imageFrame.midX, y: imageFrame.midY)
        let lineFrame = CGRect(x: imageFrame.origin.x - imageFrame.width / 4, y: imageFrame.origin.y - imageFrame.height / 4,
                                width: imageFrame.width * 1.5, height: imageFrame.height * 1.5)
    ...
    @objc func touchDown(sender: TwitterButton) {
        self.layer.opacity = 0.4
    }
    
    @objc func touchUpInside(sender: TwitterButton) {
        self.layer.opacity = 1.0
    }
    
    @objc func touchDragExit(sender: TwitterButton) {
        self.layer.opacity = 1.0
    }
    
    @objc func touchDragEnter(sender: TwitterButton) {
        self.layer.opacity = 0.4
    }
    
    @objc func touchCancel(sender: TwitterButton) {
        self.layer.opacity = 1.0
    }
